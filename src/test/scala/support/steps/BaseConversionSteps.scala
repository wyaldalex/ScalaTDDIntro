...
import com.tudux.tdd.intro.converter.BaseConversion
...
class BaseConversionSteps extends BaseSteps  {
  ...
  When(
    """^I convert it to (binary|decimal) using the BaseConversion utility$""") { (x: String) =>
    baseTo = if (x == "binary")
      BaseConversion.decimalToBinary(baseFrom) else
      BaseConversion.binaryToDecimal(baseFrom)
  }
  Then(
    """^I get back a (binary|decimal) number (\d+)$""") { (x: String, expected: String) =>
